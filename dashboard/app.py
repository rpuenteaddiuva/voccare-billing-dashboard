...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, 'scripts'))
...
# Use absolute path relative to the script location to be safe
...
if not files:
    st.sidebar.error(f"No CSV files found in: {input_dir}")
...
if selected_country != "Todos (Global)":
    ...
    try:
        ...
        header = pd.read_csv(f_path, sep=';', nrows=0)
        col_name = None
        ...
        if col_name:
            ...
            acc_df = pd.read_csv(f_path, sep=';', usecols=[col_name], on_bad_lines='skip')
...
with tab_data:
    ...
    if not df_view.empty:
        ...
        # Reordenar columnas para poner los totales al principio o final si se desea
        # Por ahora las dejamos al final por defecto
